...
cat $TMPE/factures.csv|grep ";ECHEANCE;"|while read line; do cp $LATEX/$(ls $LATEX/|grep $(echo $line|cut -d";" -f4)"_"$(echo $line|cut -d";" -f2|sed 's/-//g')|tail -n1) $TMPE/pdf/$(echo $line|cut -d";" -f4).pdf; done
