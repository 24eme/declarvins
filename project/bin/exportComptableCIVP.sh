...
awk -F";" -v OFS=';' -v ibancivp="$IBANCIVP" -v biccivp="$BICCIVP" -v banquecivp="$BANQUECIVP" 'function esc(v) { gsub(/"/, "", v); gsub(/;/, ",", v); sub(/^[[:space:]]+/, "", v); sub(/[[:space:]]+$/, "", v); return v; } function escBanque(v) { gsub(/[\047]/, " ", v); return v; } function escTVA(v) {  return (tolower(v) ~ /^fr[[:digit:]]{11}$/) ? v : "" } function generateCompte(v) { sub(/^/, "411", v); return v; } NR>1 {print $1,esc($2),($12 ? substr($12, 1, 9) : ""), generateCompte($1),(esc($5) ? esc($5) : esc($2)),$7,esc($8),$9,$17,$15,$1,"","",escTVA($11),"fr_FR","Paiement sous 60 jours",($24 ? $1 : ""),$14,"Récurrent",$24,$23,escBanque(esc($21)),($24 ? "" : ibancivp),($24 ? "" : biccivp),($24 ? "" : banquecivp),""}' $TMPE/societes.csv >> $TMPE/pennylane.societes.csv
