...
echo "Identifiant Client;Dénomination;Siren;Numéro de Compte;Adresse;Code Postal;Ville;Pays;Liste d’e-mails;Téléphone;Référence Client;Destinataire;Remarques;Numéro de TVA;Langue de Facturation;Conditions de Paiement;Prélèvement : Numéro de Mandat (RUM);Prélèvement : Date de Signature;Prélèvement : Type de Paiement;Prélèvement : IBAN;Prélèvement : BIC;Prélèvement : Banque;Virement : IBAN;Virement : BIC;Virement : Banque;ID du mandat de GoCardless" > $TMPE/pennylane.societes.csv
awk -F";" -v OFS=';' -v ibancivp="$IBANCIVP" -v biccivp="$BICCIVP" -v banquecivp="$BANQUECIVP" 'function esc(v) { gsub(/"/, "", v); gsub(/;/, ",", v); sub(/^[[:space:]]+/, "", v); sub(/[[:space:]]+$/, "", v); return v; } function escBanque(v) { gsub(/[\047]/, " ", v); return v; } function escTVA(v) {  return (tolower(v) ~ /^fr[[:digit:]]{11}$/) ? v : "" } function generateCompte(v) { sub(/^/, "411", v); return v; } NR>1 {print $1,esc($3),($13 ? substr($13, 1, 9) : ""), generateCompte($1),(esc($6) ? esc($6) : esc($3)),$8,esc($9),$10,$18,$16,$1,"","",escTVA($12),"fr_FR","Paiement sous 60 jours",($25 ? $1 : ""),$15,"Récurrent",$25,$24,escBanque(esc($22)),($25 ? "" : ibancivp),($25 ? "" : biccivp),($25 ? "" : banquecivp),""}' $TMPE/societes.csv >> $TMPE/pennylane.societes.csv
...
cat $TMPE/factures.csv | php bin/convertExportFacture2PennyLane.php > $TMPE/pennylane.factures.csv
