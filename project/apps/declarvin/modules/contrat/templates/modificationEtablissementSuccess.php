    ...
    foreach ($contrat->etablissements as $etablissement):
        if ($etablissement->getKey() == $form->getObject()->getKey()):
            ...
            <form id="creation_compte" method="post" action="<?php echo ($recapitulatif) ? url_for('contrat_etablissement_modification', array('indice' => $etablissement->getKey(), 'recapitulatif' => 1)) : url_for('contrat_etablissement_modification', array('indice' => $etablissement->getKey())); ?>">
                ...
                <h2>
                    <?php
                        $etablissementTitle = "Veuillez saisir les informations pour ";
                        $etablissementTitle.= (count($contrat->etablissements)===1)? "votre établissement" : "";
                        $etablissementTitle.= (count($contrat->etablissements)>1 && ($etablissement->getKey() == 0))? "le 1er établissement" : "";
                        $etablissementTitle.= (count($contrat->etablissements)>1 && ($etablissement->getKey() > 0))? "le ".($etablissement->getKey() + 1)."ème établissement" : "";
                        echo $etablissementTitle;
                    ?>
                </h2>
