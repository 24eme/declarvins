<section id="contenu" style="padding: 30px 20px 70px;">
	<h1>Erreur lors de la connexion</h1>
	<p id="erreur_connexion">
	Vous tentez de vous connecter à la plateforme <a href="<?php echo url_for('ac_vin_login') ?>" class="lien_declarvin">Declarvins.net</a> réservée aux déclarant avec votre compte partenaire EDI.<br />
	Merci de bien vouloir contacter votre interpro si cela vous pose problème.<br /><br />
	Bonne navigation.<br /><br />
	L'équipe Declarvins.net
	</p>
	<br />
	<div id="btn_etape_dr">
	<a href="<?php echo url_for('@ac_vin_logout') ?>" class="btn_prec"><span>Retour à la page de login</span></a>
	</div>
</section>