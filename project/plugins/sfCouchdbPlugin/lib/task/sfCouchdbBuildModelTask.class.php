...
class sfCouchdbBuildModelTask extends sfBaseTask {
...
The [sfCouchdbBuildModel|INFO] task does things.
Call it with:

  [php symfony sfCouchdbBuildModel|INFO]
EOF;
    }
    ...
    protected function execute($arguments = array(), $options = array()) {
        ...
        $definition = sfCouchdbManager::getDefinition($model);
    ...
    protected function generateTreeClasses($dir, $base_dir, $definition) {
        ...
        foreach ($fields as $field) {
            if ($field->isCollection() && $field->getCollectionClass() != 'sfCouchdbJson') {
                ...
                file_put_contents($base_dir . '/' . $base_filename, $this->getFileContentBaseTree(array("%MODEL_NAME%" => $definition->getModel(),
                            ...
                            "%EXTENDS%" => ($field->getCollectionInheritance()) ? $field->getCollectionInheritance() : 'sfCouchdbDocumentTree',
...
class %INHERITANCE_MODEL_NAME% extends sfCouchdbDocumentTree {
