                        ...
                        <td style="background: #f1f1f1;border: none;text-align: left;">
                            ...
                            <dialog id="modale_<?php echo $p->getIdentifiantHTML() ?>">
                                <form method="post" action="<?php echo url_for('drm_update_reserve_produit', $drm) ?>?millesime=<?php echo $p->getMillesimeForReserveInterpro() ?>">
                                    ...
                                    <p style="padding: 5px 0;">Le volume en réserve du <strong><?php echo $p->getFormattedLibelle(ESC_RAW); ?></strong> millésime <strong><?php echo $p->getMillesimeForReserveInterpro() ?></strong> est de <strong><?php echoFloat($p->getReserveInterpro()); ?></strong>&nbsp;hl</p>
                            ...
                            <td style="background: #f1f1f1;border: none;text-align: left;">
                                ...
                                <dialog id="modale_<?php echo $p->getIdentifiantHTML() ?>_<?php echo $millesime ?>">
                                    <form method="post" action="<?php echo url_for('drm_update_reserve_produit', $drm) ?>?millesime=<?php echo $millesime ?>">
                                        ...
                                        <p style="padding: 5px 0;">Le volume en réserve du <strong><?php echo $p->getFormattedLibelle(ESC_RAW); ?></strong> millésime <strong><?php echo $millesime ?></strong> est de <strong><?php echoFloat($volume); ?></strong>&nbsp;hl</p>
                                        <p style="padding: 5px 0;"><label for="reserve">Nouveau volume en reserve pour le millésime <?php echo $millesime ?> : </label><input id="reserve" type="text" inputmode="numeric" name="reserve" required />&nbsp;hl</p>
                                        <p style="padding: 5px 0;text-align: center;"><input type="submit" name="submit" value="Valider" /></p>
                                    </form>
                                </dialog>
                            </td>
