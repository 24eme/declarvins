<?php
class ConfigurationProduitDefinitionValidatorSchema extends sfValidatorSchema
{
    ...
    public function configure($options = array(), $messages = array()) 
    {
  	...
  	{ 
	  	...
	  	if (isset($values['droit_douane'])) {
		    ...
		    {
		      if (!$value['date'] && !$value['code'] && !$value['date'])
		      {
	  	...
	  	if (isset($values['organismes'])) {
		    foreach($values['organismes'] as $key => $value)
		    {
		      if (!$value['organisme'] && !$value['organisme']['date'])
		      {
		        unset($values['organismes'][$key]);
		      }
		    }
	  	}
	    return $values;
