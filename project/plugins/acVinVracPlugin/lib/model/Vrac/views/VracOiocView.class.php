<?php
class VracOiocView extends acCouchdbView
{
	const KEY_OIOC_ID = 0;
	const KEY_OIOC_STATUT = 1;
	const KEY_VRAC_TYPE = 2;
	const KEY_DATES_SAISIE = 3;
	const KEY_VRAC_ID = 4;
	...
	{
        return acCouchdbManager::getView('vrac', 'oioc', 'Vrac');
    }

    public function findByOiocAndDate($oioc, $statut, $type, $date) 
    {
      	return $this->client->startkey(array($oioc, $statut, $type, $date))
                    		->endkey(array($oioc, $statut, $type, $this->getEndISODateForView(), array()))
