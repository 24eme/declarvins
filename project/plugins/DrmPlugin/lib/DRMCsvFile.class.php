...
class DRMCsvFile extends CsvFile 
{
  ...
  const CSV_COL_CONTRAT_VOLUME = 14;
  ...
  private function getProduit($line) {
    if($this->drm->annee != $line[self::CSV_COL_DETAIL_ANNEE])
      throw new sfException("Incoherence dans l'année de la DRM");
    if($this->drm->mois != $line[self::CSV_COL_DETAIL_MOIS])
      throw new sfException("Incoherence dans le mois de la DRM");
    if($this->drm->identifiant != $line[self::CSV_COL_DETAIL_IDENTIFIANT_DECLARANT])
      throw new sfException("Incoherence dans l'identifiant de l'établissement DRM");
    
    $hash = $this->config->identifyProduct($line[self::CSV_COL_DETAIL_CERTIFICATION], 
					   $line[self::CSV_COL_DETAIL_APPELLATION], 
					   $line[self::CSV_COL_DETAIL_LIEU], 
					   $line[self::CSV_COL_DETAIL_COULEUR], 
					   $line[self::CSV_COL_DETAIL_CEPAGE], 
					   $line[self::CSV_COL_DETAIL_MILLESIME]);
    return $this->drm->addProduit($hash, $this->config->identifyLabels($line[self::CSV_COL_DETAIL_LABELS]))->getDetail();
  }

  private function parseContrat($line) {
    $detail = $this->getProduit($line);
    $detail->add('vrac')->add($line[self::CSV_COL_CONTRAT_IDENTIFIANT], $line[self::CSV_COL_CONTRAT_VOLUME]);
    $this->drm->update();
  }

  private function parseDetail($line) {
      $detail = $this->getProduit($line);
      if ($line[self::CSV_COL_DETAIL_MENTION])
	$detail->label_supplementaire = $line[self::CSV_COL_DETAIL_MENTION] * 1;
  ...
  public function importDRM($etablissement) {
	...
	$this->numline++;
	if (!$this->drm)
	  $this->drm = DRMClient::getInstance()->retrieveOrCreateByIdentifiantAndCampagne($etablissement, $line[self::CSV_COL_DETAIL_ANNEE], $line[self::CSV_COL_DETAIL_MOIS]);
