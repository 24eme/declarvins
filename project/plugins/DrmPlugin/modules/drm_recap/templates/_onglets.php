 ...
 <div id="msg_aide_drm">
    ...
    <?php foreach ($items as $item): ?>
        <?php if ($item->hasMouvementCheck()): ?>
            <?php if ($item->getKey() == $drm_appellation->getKey()): ?>
                <li class="actif">
                    <strong>
                        <?php echo $item->getConfig()->libelle ?> 
            ...
            <?php else: ?>
                <li>
                    <a title="<?php echo $item->getConfig()->libelle ?>" href="<?php echo url_for('drm_recap_appellation', $item) ?>">
                        <?php echo $item->getConfig()->code ?> 
                        <span class="completion<?php if($item->isComplete()): ?> completion_validee<?php endif; ?>">
                            (<span class="appellation_produit_saisie"><?php echo $item->nbComplete() ?></span>/<span class="appellation_produit_total"><?php echo $item->nbToComplete() ?></span>)
                        </span>
                    </a>
    ...
    <?php endforeach; ?>
    <li class="ajouter">
        <a href="<?php echo url_for('drm_recap_appellation_ajout_ajax', $drm_appellation->getCertification()) ?> "class="btn_popup" data-popup="#popup_ajout_appelation" data-popup-ajax="true" data-popup-config="configForm">
