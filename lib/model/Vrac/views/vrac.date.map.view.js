function(doc) { 
	if (doc.type == "Vrac" && doc.valide.date_saisie) { 
		...
		var vrac_cas_particulier = doc.cas_particulier_libelle;
		...
		var vrac_conditions_paiement = doc.conditions_paiement_libelle;
		var vrac_reference_contrat_pluriannuel = doc.reference_contrat_pluriannuel;
		var vrac_vin_livre = doc.vin_livre;
		var vrac_date_debut_retiraison = dateEnglishFormat(doc.date_debut_retiraison);
		var vrac_date_limite_retiraison = dateEnglishFormat(doc.date_limite_retiraison);
		...
		if (nbItem > 0) {
			...
			for (p in doc.paiements) {
				counter++;
				vrac_paiements_date = vrac_paiements_date + dateEnglishFormat(doc.paiements[p].date);
			...
			for (l in doc.lots) {
				...
				if (nbItem > 0) {
					...
					for (c in doc.lots[l].cuves) {
						...
						vrac_lot_cuves_date = vrac_lot_cuves_date + dateEnglishFormat(doc.lots[l].cuves[c].date);
						if (counter != nbItem) {
				...
				emit([doc.interpro, doc.valide.date_saisie, doc._id, doc.produit], 
                		[vrac_id,
                		 vrac_date_saisie,
                		 vrac_acheteur_id,
                		 vrac_acheteur_cvi,
