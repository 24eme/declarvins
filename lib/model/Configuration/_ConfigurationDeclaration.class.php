...
abstract class _ConfigurationDeclaration extends acCouchdbDocumentTree {
    ...
    protected function setDroitCvoCsv($datas, $code_applicatif) {
    	...
    	$code = ($datas[ProduitCsvFile::CSV_PRODUIT_CVO_CODE])? $datas[ProduitCsvFile::CSV_PRODUIT_CVO_CODE] : DRMDroits::DROIT_CVO;
    ...
    public abstract function setDonneesCsv($datas);
 	...
 	public abstract function hasDroits();
  	...
  	public abstract function hasHasVrac();
  	...
  	public function getGerantInterpro() 
  	{
  		if ($this->exist('interpro')) {
  			foreach ($this->interpro as $interpro => $i) {
  				if ($i->exist('droits')) {
  					if ($i->droits->exist('cvo')) {
  						if (count($i->droits->cvo) > 0) {
  							return InterproClient::getInstance()->find($interpro);
  						}
  					}
  				}
  			}
