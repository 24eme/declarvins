    		...
    		if (doc.declaration.certifications[c].genres[g].hasOwnProperty("interpro"))
    		{
    			...
    			if (nb > 0) {
		        	...
		        	for (i in doc.declaration.certifications[c].genres[g].interpro)
		        	{
		        		...
		        		if (doc.declaration.certifications[c].genres[genre].interpro[i].hasOwnProperty("droits"))
		        		{
	                                ...
	                                if (inter[i].length > 0) {
	                                    for(array_intepro_key in inter) {
	                                        for(array_dep_key in dep) {
	                                            if (dep[array_dep_key].length > 0) {
	                                                for(d in dep[array_dep_key]) {
	                                                    emit(["produits", inter[i][array_intepro_key], c, dep[array_dep_key][d], hash_lieu, hash, codes.join('')], {libelles: libes, codes: codesv, cvo: interprosLieu[inter[i]]["cvo"]});
	                                                }
	                                                break;
	                                            }
	                                        }
