function(doc) { 
	if (doc.type == "DRM" && doc.valide.date_saisie) { 
		...
		if (!drm_identifiant_drm_historique) {
			if (drm_version) {
				drm_identifiant_drm_historique = doc._id;
			} else {
			...
			for(certification_key in doc.declaration.certifications) {
		        ...
		        if (genre_code == key_default) {
			        ...
			        if (appellation_code == key_default) {
						...
						var couleur_code = getCouleur(couleur_key);
				        ...
				        if (couleur_code == key_default) {
							...
							if (nb_labels > 0) {
			                                ...
			                                emit([doc.interpros[interpro], detail.has_vrac, doc.valide.date_saisie, detail.has_vrac, doc._id, detail_hash, "PRODUIT"], 
			                                		[key,
			                                		 drm_identifiant,
			                                		 drm_declarant,
			                                		 drm_annee,
			                                ...
			                                if (nb_vracs > 0) {
				                                for (numero_vrac in detail.vrac) {
				                                	...
				                                	emit([doc.interpros[interpro], detail.has_vrac, doc.valide.date_saisie, detail.has_vrac, doc._id, detail_hash, "VRAC"], 
					                                		[key_vrac,
					                                		 drm_identifiant,
					                                		 drm_declarant,
					                                		 drm_annee,
