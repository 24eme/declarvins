function(doc) { 
	if (doc.type == "DRM" && doc.valide.date_saisie) { 
		...
		var groupByLastVersion = function (version) {
			var versionRegexp = /[A-Z]{1}[0-9]+[A-Z]{1}[0-9]+/;
			var matches = version.match(versionRegexp);
			if (matches) {
		...
		if (!drm_identifiant_drm_historique) {
			if (drm_version) {
				drm_identifiant_drm_historique = 'DRM-'+drm_identifiant+'-'+drm_annee+'-'+drm_mois+'-'+groupByLastVersion(drm_version);
			} else {
							...
							if (nb_labels > 0) {
			                                ...
			                                emit([detail.interpro, doc.valide.date_saisie, detail.has_vrac, doc._id, detail_hash, "PRODUIT"], 
			                                		[key,
			                                		 ...
			                                		 drm_annee,
			                                		 drm_mois,
			                                		 groupByLastVersion(drm_version),
