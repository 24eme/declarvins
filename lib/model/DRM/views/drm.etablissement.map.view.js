function(doc) { 
	if (doc.type == "DRM" && doc.valide.date_saisie) { 
        ...
        var drm_commentaire = (drm.commentaires)? (drm.commentaires).replace(regexp, " ") : null;
			        ...
			        if (appellation_code == key_default) {
				        ...
				        if (couleur_code == key_default) {
							...
							if (nb_labels > 0) {
			                                ...
			                                emit([doc.identifiant, doc.valide.date_saisie, detail.has_vrac, doc._id, detail_hash, "PRODUIT"], 
			                                		[key,
			                                		 drm_identifiant,
			                                		 drm_declarant,
			                                		 drm_annee,
			                                ...
			                                if (nb_vracs > 0) {
				                                for (numero_vrac in detail.vrac) {
				                                	...
				                                	emit([doc.identifiant, doc.valide.date_saisie, detail.has_vrac, doc._id, detail_hash, "VRAC"], 
					                                		[key_vrac,
					                                		 drm_identifiant,
					                                		 drm_declarant,
					                                		 drm_annee,
