...
class DRM extends BaseDRM implements InterfaceVersionDocument {
    ...
    public function setDroits() 
    {
        ...
        foreach ($this->getDetails() as $detail) {
        	...
        	if ($droitDouane) {
        		$this->droits->getOrAdd(DRMDroits::DROIT_DOUANE)->getOrAdd($droitDouane->code)->integreVolume($detail->sommeLignes(DRMDroits::getDouaneDroitSorties()), $detail->sommeLignes(DRMDroits::getDroitEntrees()), $droitDouane->taux, $this->getReportByDroit(DRMDroits::DROIT_DOUANE, $droitDouane), $droitDouane->libelle);
        	}
        }
    }
